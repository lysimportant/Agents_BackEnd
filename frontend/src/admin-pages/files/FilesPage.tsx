...
export function FilesPage(props: FilesPageProps) {
  ...
  const fileFormContent = (mode: 'upload' | 'edit', asForm = true) => {
    ...
    const fields = <>
      ...
      <label>标签<Select mode="tags" tokenSeparators={[',', '，', ';', '；']} maxCount={12} value={fileForm.tags} placeholder="输入标签后按回车，最多 12 个" onChange={(tags) => onFileFormChange({ ...fileForm, tags })} /></label>
