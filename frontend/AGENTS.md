...
当前目录是已运行的 B 端管理前端，不是 `portal/` C 端。它同时承载后台工作台、内部聊天真实路由和 Socket 客服访客/管理入口，但三者必须保持各自状态与鉴权边界。
...
- 文件管理批量上传按文件顺序发送，失败文件保留在待上传列表以便重试；标签最多 12 个、每个最多 24 个 Unicode 字符，前端展示应与后端规范化结果一致。`MaxMultipartMemory=32 MiB` 只是 multipart 解析内存阈值，不是文件大小上限。
- 受保护缩略图可使用 `.thumbnail-cache`，列表不应因缓存预加载阻塞首屏；已加载图片不主动卸载，只有未完成且离开预加载范围的请求可以取消。
