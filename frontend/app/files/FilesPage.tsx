...
export function FilesPage(props: FilesPageProps) {
  ...
  const clampScale = (next: number) => Math.min(5, Math.max(0.35, Number(next.toFixed(2))));
  ...
  const fileFormContent = (mode: 'upload' | 'edit', asForm = true) => {
    ...
    return asForm ? <form className="antd-file-form" id={`file-${mode}-form`} onSubmit={onSubmitFile}>{fields}</form> : <div className="antd-file-form">{fields}</div>;
  };

  return (
    <div className="page-stack files-workspace antd-files-workspace">
      <Card className="files-hero" bordered={false}>
        <div><p className="page-kicker">内容管理 / 资源中心</p><h1>文件管理</h1><span>卡片仅加载压缩缩略图；点击图片才请求原图。PDF 可直接交给浏览器阅读。</span></div>
        <div className="file-hero-stats"><strong>{files.length}</strong><span>文件总数</span><small>{formatFileSize(totalSize)}</small></div>
      </Card>

      <Card className="file-browser-panel" title="文件库" extra={<div className="antd-file-tools"><Input value={fileKeyword} allowClear onChange={(event) => onFileKeywordChange(event.target.value)} placeholder="名称、分类或说明" prefix={<FileTextOutlined />} /><Button onClick={() => onFileKeywordChange('')}>重置</Button><Button type="primary" icon={<InboxOutlined />} onClick={() => { onResetFileForm(); setIsUploadOpen(true); }}>上传文件</Button><Button icon={<DeleteOutlined />} onClick={() => void openRecycleBin()}>回收站{recycleFiles.length ? ` (${recycleFiles.length})` : ''}</Button></div>}>
        <div className="file-type-tabs" role="tablist">{FILE_KIND_OPTIONS.map((item) => <button className={activeKind === item.key ? 'active' : ''} type="button" key={item.key} onClick={() => setActiveKind(item.key)}><span>{item.icon}</span>{item.label}<strong>{kindCounts[item.key]}</strong></button>)}</div>
        {visibleFiles.length === 0 ? <Empty description="暂无匹配文件" /> : <div className="file-card-grid">{visibleFiles.map((file) => <FileCard key={file.id} file={file} onOpenImage={openImage} onEditFile={openEditDialog} onDownloadFile={onDownloadFile} onDeleteFile={onDeleteFile} />)}</div>}
      </Card>

      <Modal open={isUploadOpen} title="上传文件" okText="上传" cancelText="取消" confirmLoading={isSavingFile} onOk={() => document.getElementById('file-upload-form')?.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }))} onCancel={closeUploadDialog} destroyOnClose>
        {fileFormContent('upload')}
      </Modal>

      <Modal open={isEditOpen} title="编辑文件信息" okText="保存" cancelText="取消" confirmLoading={isSavingFile} onOk={() => document.getElementById('file-edit-form')?.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }))} onCancel={closeEditDialog} destroyOnClose>
        {fileFormContent('edit')}
      </Modal>

      <Drawer title="文件回收站" open={isRecycleOpen} width={620} onClose={() => { setIsRecycleOpen(false); setRecycleError(''); }} extra={<Button loading={isRecycleLoading} onClick={() => void openRecycleBin()}>刷新</Button>}>
      ...
      <Modal open={Boolean(textEditorFile)} title={`编辑文本文件：${textEditorFile?.displayName ?? ''}`} okText="保存全部" cancelText="关闭" width="min(1040px, 96vw)" confirmLoading={isTextSaving || isSavingFile} onOk={() => void saveTextContent()} onCancel={() => { setTextEditorFile(null); onResetFileForm(); }} destroyOnClose>
      ...
      <Modal className="file-image-zoom-modal" open={Boolean(previewFile)} title={previewFile?.displayName} footer={null} width="min(1500px, 98vw)" centered styles={{ body: { padding: 0 } }} onCancel={() => setPreviewFile(null)} destroyOnClose>
        {previewFile && <div className="file-image-zoom-wrap"><div className="file-image-zoom-toolbar"><span>打开默认显示完整图片；滚轮缩放，放大后可拖拽移动</span><Space><Button icon={<ZoomOutOutlined />} onClick={() => setImageScale((current) => clampScale(current - 0.25))}>缩小</Button><strong>{Math.round(imageScale * 100)}%</strong><Button icon={<ZoomInOutlined />} onClick={() => setImageScale((current) => clampScale(current + 0.25))}>放大</Button><Button icon={<CompressOutlined />} onClick={resetImageTransform}>适配</Button></Space></div><div className={`file-image-zoom-stage ${isDraggingImage ? 'dragging' : ''}`} onWheel={onPreviewWheel} onMouseDown={startImageDrag} onMouseMove={moveImageDrag} onMouseUp={stopImageDrag} onMouseLeave={stopImageDrag}>{originalLoading && <div className="original-image-loading"><LoadingOutlined spin /> 正在加载原图…</div>}<img draggable={false} src={`${API_BASE_URL}/api/files/${previewFile.id}/preview`} alt={previewFile.displayName} style={{ transform: `translate(${imageOffset.x}px, ${imageOffset.y}px) scale(${imageScale})` }} onLoad={() => setOriginalLoading(false)} onError={() => setOriginalLoading(false)} /></div></div>}
      </Modal>
    </div>
  );
}

type FileCardProps = { file: ManagedFile; onOpenImage: (file: ManagedFile) => void; onEditFile: (file: ManagedFile) => void; onDownloadFile: (fileId: number) => void; onDeleteFile: (fileId: number) => void };
function FileCard({ file, onOpenImage, onEditFile, onDownloadFile, onDeleteFile }: FileCardProps) {
  ...
  return <article className={`file-card tone-${meta.tone}`}>
    <div className="file-preview-frame">
      {isImage ? <button className="thumbnail-button" type="button" onClick={() => onOpenImage(file)} aria-label={`预览原图：${file.displayName}`}><img src={thumbnailUrl} alt={`${file.displayName} 缩略图`} loading="lazy" /><span><EyeOutlined /> 查看原图</span></button> : isPDF ? <a className="file-preview-icon pdf-preview" href={previewUrl} target="_blank" rel="noopener"><FilePdfOutlined /><strong>PDF</strong><small>点击浏览</small></a> : <div className="file-preview-icon">{isImage ? <PictureOutlined /> : <FileImageOutlined />}<span>{meta.icon}</span><strong>{meta.label}</strong><small>{getFileExtension(file.originalName).toUpperCase() || meta.description}</small></div>}
    </div>
    <div className="file-card-body"><div className="file-card-title"><strong title={file.displayName}>{file.displayName}</strong><Space size={4} wrap><Tag>{file.category || '未分类'}</Tag><Tag color={file.isPrivate ? 'warning' : 'blue'}>{file.isPrivate ? '私密' : '公开'}</Tag></Space></div><p title={file.originalName}>{file.originalName}</p><small>{file.description || '暂无说明'}</small><div className="file-meta-row"><span>归属：{file.ownerName || '未知'}</span><span>{formatFileSize(file.size)}</span><span>{new Date(file.updatedAt).toLocaleString()}</span></div></div>
    <div className="file-card-actions">
      ...
      <Button type="link" icon={<EditOutlined />} onClick={() => onEditFile(file)}>编辑</Button><Button type="link" icon={<DownloadOutlined />} onClick={() => onDownloadFile(file.id)}>下载</Button><Popconfirm title="确认将该文件移入回收站？可通过恢复接口找回。" okText="移入回收站" cancelText="取消" onConfirm={() => onDeleteFile(file.id)}><Button danger type="link" icon={<DeleteOutlined />}>移入回收站</Button></Popconfirm>
    </div>
  </article>;
}
