...
- Compose 镜像更新若可能导致 B 端 WebSocket 断开，应由宿主机 `systemd-run --no-block` 动态 oneshot 单元执行 `git pull`、`docker compose config --quiet`、`docker compose up -d --build` 和 `docker compose ps`，再用 `journalctl -u <unit>` 查看完整结果；部署机直连本身不使用 SSH/22 端口。
...
- 文件管理上传不设置应用层单文件大小上限，并按同一所有者的 SHA-256 内容哈希过滤有效重复文件；聊天与文章附件仍遵循各自限制。服务端存储名必须安全生成；下载、预览和删除通过文件 ID 解析数据库记录，禁止接受任意磁盘路径。
- `DELETE /api/files/:id` 只设置 `deleted_at`；只有回收站中的文件才能调用永久删除并移除物理文件。
- 文件管理标签写入前会去首尾空白、去除前导 `#` 并按不区分大小写去重，最多 12 个、每个最多 24 个 Unicode 字符；同批多文件上传由前端编排，后端以完整内容哈希返回 `409 DUPLICATE_FILE`。
- `/api/files/:id/thumbnail` 的受保护缩略图可落在可再生成的 `.thumbnail-cache`，缓存只占用磁盘并可重建，不是业务上传文件；删除文件仍先软删除，永久删除才移除原文件。
