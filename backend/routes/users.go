...
func registerUserRoutes(routes *gin.RouterGroup, store middleware.UserStore, handler *handlers.UserHandler) {
	...
	routes.POST("/profile/password-code", handler.SendPasswordCode)
	routes.PUT("/profile/password", handler.ChangeCurrentPassword)
	...
	routes.PUT("/users/:id/actions", middleware.RequireAdmin(), handler.UpdateActions)
