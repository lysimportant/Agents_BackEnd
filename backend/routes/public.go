...
func registerPublicRoutes(routes *gin.RouterGroup, handler *handlers.PublicHandler) {
	...
	public.GET("/dailies", handler.ListDailies)
	public.POST("/dailies/media", handler.UploadDailyMedia)
	public.GET("/dailies/:id", handler.GetDaily)
	public.GET("/dailies/:id/interactions", handler.GetDailyInteraction)
	public.POST("/dailies/:id/like", handler.ToggleDailyLike)
	public.POST("/dailies/:id/comments", handler.CreateDailyComment)
	public.POST("/dailies", handler.CreateDaily)
