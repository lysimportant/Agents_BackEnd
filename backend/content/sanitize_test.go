...
func TestSanitizeDailyContent(t *testing.T) {
	sanitizedHTML := SanitizeDailyContent(`<p><em>记录</em></p><img src="https://example.com/a.jpg"><video src="https://example.com/a.mp4"></video>`)
	if strings.Contains(sanitizedHTML, "<img") || strings.Contains(sanitizedHTML, "<video") {
		t.Fatalf("daily HTML should not contain media tags: %q", sanitizedHTML)
	}
