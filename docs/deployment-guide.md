...
终端浏览器通道和宿主机代理注册通道都使用 Ping/Pong 保活；普通 SSH 会话还会发送 `keepalive@openssh.com` 应用层保活。反向代理必须保留 WebSocket `Upgrade`，并将读取超时设置得明显高于心跳窗口（默认心跳间隔约 25 秒、失效检测窗口约 75 秒）。页面仍打开且未主动断开时，浏览器会对明确标记为传输故障的断线按退避策略重建通道；正常 shell 结束不会自动创建新会话。
