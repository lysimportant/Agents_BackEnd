...
C 端也可以复用后端登录会话。只有 B 端标记为 18R 且 C 端登录后开启“显示 18R 内容”时，公开接口才会包含该内容；匿名、未登录或关闭 C 端开关时不会返回 B 端标记的 18R 内容。开关通过 `/api/auth/portal-r18` 写入后端域 `portal-r18=1` HttpOnly Cookie，会话 ID 始终保存在 HttpOnly Cookie 中。
...
终端 WebSocket 使用服务端 Ping/Pong 保活，浏览器切换到后台标签页、最小化终端弹窗或切换管理页面不会主动释放连接。页面仍打开且未主动断开时，短暂网络抖动会按退避策略自动重建浏览器通道和部署机 PTY；服务端以 `retryable=true` 标记传输故障，正常 shell 退出不会自动创建新会话。目录、搜索、文件读取和保存请求可携带唯一 `requestId`，服务端及代理在成功或错误响应中原样回显，前端据此忽略过期或跨通道响应。退出登录、关闭终端标签或关闭浏览器页面才会释放会话。浏览器或操作系统完全冻结进程、宿主机代理停止或远端 SSH 服务本身结束时，无法保证原有 PTY 状态不变，恢复后会建立新的会话。
...
需要先在 B 端为资源开启 18R，再在 C 端登录有效账号并开启 18R 开关。仅开启 C 端开关不会显示未标记为 18R 的内容之外的额外资源；只有后端域 `portal-r18=1` 而没有有效会话时，后端仍会排除 B 端标记的 18R 内容。
