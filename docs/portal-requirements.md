...
C 端不得调用后台文章、文件、用户、权限或终端写接口。公开图片点赞、追加标签和评论写入由 `/api/public/*` 在后端校验登录会话；后端 HttpOnly Cookie 是唯一会话凭据，浏览器不得保存会话 ID、密码或访问令牌。
...
文件或文章在 B 端开启 18R 标记后，后端只有在以下两个条件同时成立时才包含该内容：

1. 请求携带有效后端登录会话 Cookie。
2. 后端域请求携带由 `POST /api/auth/portal-r18` 写入的 `portal-r18=1` Cookie。

仅设置 18R Cookie、仅登录或匿名访问都不会返回 B 端标记的 18R 内容；B 端未开启 18R 的公开内容不受 C 端开关影响。偏好由后端域 Cookie 保存，避免 C 端与后端跨域部署时 Cookie 无法随公开请求发送；退出登录会清除该偏好。C 端服务端渲染页面会转发当前认证 Cookie，登录用户的结果不使用共享缓存。
...
| `/{locale}/daily` | 日常列表；通过按钮进入独立发布页。 |
| `/{locale}/daily/publish` | 登录用户富文本编辑和发布确认；页面不进入搜索索引。 |
...
- 日常发布要求有效登录会话，正文使用白名单清洗后的富文本 HTML 保存，用户可见文字最多 2000 个 Unicode 字符；公开日常匿名可读，私密日常仅发布人本人登录后可读。详情读取会增加一次浏览量。
- 日常可选用匿名可见的公开图片作为封面；未指定时由后端随机选择公开图片。日常列表和详情返回受控封面地址、浏览量和点赞量，封面不会暴露文件编号或存储名。
- 日常互动读取允许匿名；点赞和评论要求有效登录。每个账号对每条日常最多一个点赞，再次调用会取消；评论为去首尾空白的纯文本且最多 500 个 Unicode 字符。
- 日常发布页支持登录用户通过 `/api/public/dailies/media` 上传常见图片和视频，编辑器只保存经后端校验的公开预览地址；SVG、HTML、脚本和其他不可确认类型拒绝上传。
...
- 队列取消只针对尚未完成的预加载请求；已经显示的卡片和已经加载完成的图片继续保留，不因为滚动离开视野而卸载，避免回滚时重新请求和瀑布流闪烁。
- 当前预览浮层一次只展示并请求当前原图，提供复制地址、下载原图、点赞、追加标签与评论区。匿名用户点击标签或评论写入控件时显示登录 message，不发送写请求。未来漫画阅读应创建独立队列，在视口附近逐张加载原图并预加载下一张，禁止无界并发。
